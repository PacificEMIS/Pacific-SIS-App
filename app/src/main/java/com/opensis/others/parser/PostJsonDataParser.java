...
public class PostJsonDataParser {
   ...
   */
    ...
    public PostJsonDataParser(final Context context, int method, String url, JSONObject jsonRequest, final boolean flag, View view, final OnGetResponseListner listner) {
        // final SharedPreferenceHelper helper = SharedPreferenceHelper.getInstance(context);
        // AppData.sToken = helper.getUserToken();
        Pref pref=new Pref(context);
        if (!Util.isConnected(context)) {
